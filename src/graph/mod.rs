...
/// A graph which stores a list of all incoming and outgoing edges
#[derive(Default, Debug, Clone)]
pub struct DirectedGraphVec {
    edges: Vec<(i64, (SmallI32Vec, SmallI32Vec))>,
}
impl DirectedGraphVec {
    pub fn from_vertexes<'a>(verts: impl Iterator<Item = &'a i64>) -> Self {
        let mut edges = Vec::new();
        edges.extend(verts.map(|v| (*v, (smallvec![], smallvec![]))));
        edges.shrink_to_fit();

        DirectedGraphVec { edges }
    }
    #[inline]
    fn get_from_vertex(&self, v: &i64) -> Option<&(SmallI32Vec, SmallI32Vec)> {
        self.id_for_vertex(v).map(|idx| &self.edges[idx].1)
    }
    #[inline]
    fn vertex_for_id(&self, idx: &i32) -> i64 {
        self.edges[*idx as usize].0
    }
    #[inline]
    fn id_for_vertex(&self, v: &i64) -> Option<usize> {
        self.edges.binary_search_by_key(v, |(w, _edges)| *w).ok()
    }
}

impl DirectedGraphTrait for DirectedGraphVec {
    fn new() -> Self {
        Self { edges: vec![] }
    }
    fn add_edge(&mut self, vertex1: i64, vertex2: i64) -> bool {
    ...
    fn in_neighbours(&self, from_vertex: i64) -> impl Iterator<Item = i64> {
        self.get_from_vertex(&from_vertex)
            .map(|(ins, _outs)| ins)
            .into_iter()
            .flat_map(move |ins| ins.iter().map(|i| self.vertex_for_id(i)))
    }
    fn out_neighbours(&self, from_vertex: i64) -> impl Iterator<Item = i64> {
        self.get_from_vertex(&from_vertex)
            .map(|(_ins, outs)| outs)
            .into_iter()
            .flat_map(|outs| outs.iter().map(|i| self.vertex_for_id(i)))
    }

    fn num_vertexes(&self) -> usize {
        self.edges
            .par_iter()
            .map(|(_idx, (ins, _outs))| ins.len())
            .sum()
    }
    fn num_edges(&self) -> usize {
        self.edges_iter().count()
    ...
    /// True iff this vertex is in this graph
    fn contains_vertex(&self, vid: &i64) -> bool {
        self.get_from_vertex(vid).is_some()
    }
    ...
    fn edges_iter(&self) -> impl Iterator<Item = (i64, i64)> + '_ {
        self.edges
            .iter()
            .filter(|(_vid, (ins, outs))| !ins.is_empty() || !outs.is_empty())
            .flat_map(|(vid, (_ins, outs))| {
                outs.iter().map(|other| (*vid, self.vertex_for_id(other)))
            })
    }
    fn edges_par_iter(&self) -> impl ParallelIterator<Item = (i64, i64)> {
        self.edges
            ...
            .flat_map(|(vid, (_ins, outs))| {
                outs.par_iter()
                    .map(|other| (*vid, self.vertex_for_id(other)))
            })
    }
    ...
    fn vertexes_and_num_ins_outs(&self) -> impl Iterator<Item = (i64, usize, usize)> + '_ {
        self.edges
            .iter()
            .filter(|(_vid, (ins, outs))| !ins.is_empty() || !outs.is_empty())
            .map(|(vid, (ins, outs))| (*vid, ins.len(), outs.len()))
    }
    ...
    fn remove_edge(&mut self, vertex1: &i64, vertex2: &i64) {
        let vidx1: i32 = bin_search_key(vertex1, &self.edges)
            .unwrap()
            .try_into()
            .unwrap();
        let vidx2: i32 = bin_search_key(vertex2, &self.edges)
            .unwrap()
            .try_into()
            .unwrap();
    ...
    fn vertexes(&self) -> impl Iterator<Item = i64> + '_ {
        self.edges
            .iter()
            .filter(|(_vid, (ins, outs))| !ins.is_empty() || !outs.is_empty())
            .map(|(vid, (_ins, _outs))| *vid)
    }

    fn into_disconnected_graphs(
        self,
        progress_bar: &ProgressBar,
    ) -> impl Iterator<Item = DirectedGraphVec> {
        let mut g = self;
        let mut vertexes_to_look_at: Vec<i32> = Vec::new();
        let mut buf: SmallVec<[_; 2]> = SmallVec::new();

        std::iter::from_fn(move || {
            if g.edges.is_empty() {
                return None;
            }
            let num_trailing_empties = g
                .edges
                .iter()
                .rev()
                .take_while(|(_nid, (ins, outs))| ins.is_empty() && outs.is_empty())
                .count();
            g.edges.truncate(g.edges.len() - num_trailing_empties);
            //dbg!(num_trailing_empties, g.edges.capacity() - g.edges.len());
            //if g.edges.capacity() - g.edges.len() > 1_000 {
            ...
            let mut new_graph = DirectedGraph2::new();
            vertexes_to_look_at.truncate(0);
            vertexes_to_look_at.push((g.edges.len() - 1).try_into().unwrap());
            let (mut vertex, mut in_vertex, mut out_vertex);

            while let Some(vertex_idx) = vertexes_to_look_at.pop() {
                progress_bar.inc(1);
...
impl From<DirectedGraph2> for DirectedGraphVec {
    fn from(g: DirectedGraph2) -> DirectedGraphVec {
        let DirectedGraph2 { edges } = g;
        assert!(edges.len() < i32::MAX as usize);
        let mut edge_ids: Vec<i64> = Vec::with_capacity(edges.len());
        edge_ids.extend(edges.keys());

        // NB: We could reserve the whole thing, but I want to limit max memory used
        let mut new_edges: Vec<(i64, (SmallI32Vec, SmallI32Vec))> = Vec::new();

        for (vid1, (ins1, outs1)) in edges.into_iter() {
            let mut ins2 = SmallVec::with_capacity(ins1.len());
            ins2.extend(ins1.into_iter().map(|other| {
                edge_ids
                    .binary_search(&other)
            ...
            let mut outs2 = SmallVec::with_capacity(outs1.len());
            outs2.extend(outs1.into_iter().map(|other| {
                edge_ids
                    .binary_search(&other)
                    .expect("Can't find key")
                    .try_into()
            ...
            new_edges.push((vid1, (ins2, outs2)));
        }

        DirectedGraphVec { edges: new_edges }
    }
}
